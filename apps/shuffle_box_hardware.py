...
locations repeatedly. This is tested with a physical robot, without a gripper connected. Meaning 
that this application will only move between two predefined waypoints. 
The application needs some adjusments if a gripper is connected to the robot.
'''
...
def create_composite_atlas(cask_root, joints):
    ...
    quantities = [[x, "none", 1] for x in ["pump", "valve"]]
    SUCTION_ON_WAYPOINT = np.array([1.0, 0.0], dtype=np.dtype("float64"))
    SUCTION_OFF_WAYPOINT = np.array([0.0, 1.0], dtype=np.dtype("float64"))
    VALVE_OFF_WAYPOINT = np.array([0.0, 0.0], dtype=np.dtype("float64"))
